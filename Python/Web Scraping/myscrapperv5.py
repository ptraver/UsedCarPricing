# -*- coding: utf-8 -*-
"""
Created on Sat Jul  1 22:22:03 2017
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 15 21:09:59 2017
...
class DoneDealSpider(scrapy.Spider):
    ...
    def parse_item_page(self, response):
        ...
        web = urllib.request.urlopen(response.url)

        soup = BeautifulSoup(web.read(), 'lxml')
        data  = str(soup.find_all("script")[8])
        p=re.compile(r'"price":([^$]*})')
        matches = [group for group in re.findall(p, data) if group]
        p1=re.compile(r'"(displayAttributes)":([^$]*})')
        matches1 = [group for group in re.findall(p1, data) if group]
        #with open("carzone.txt", 'a+') as f:
            #f.write(str(matches[0]).split("county",1)[0]+"}"+"\n")
            
        #print(data)
        #p=re.compile(r'"(displayAttributes)":([^$]*})') #working regex
        #matches = [group for group in re.findall(p, data) if group]
        with open("DoneDeal.txt", 'a+') as f:
            f.write("{"+'"price":'+str(matches[0]).split("county",1)[0]+','+str(matches1[0]).split("views",1)[0]+"}"+"\n")
